...
main ()
{
  ...
  FILE *input_value_file = fopen("./data/dfsin/input_value.txt","a"); 
  FILE *input_size_file = fopen("./data/dfsin/input_size.txt","a");
  ...
  fprintf(input_size_file, "%d\n", SIZE);
      ...
      for (i = 0; i < SIZE; i++)
	{
	  float64 result;
      int index = rand()%30;
	  result = local_sin (test_in[index]);
      fprintf(input_value_file, "%016llx\t", test_in[index]);
	  main_result += (result != test_out[index]);

	  printf
	    ("input=%016llx expected=%016llx output=%016llx (%lf)\n",
	     test_in[index], test_out[index], result, ullong_to_double (result));
	}
      printf ("%d\n", main_result);
      return main_result;
    }
