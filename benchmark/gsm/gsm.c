...
main ()
{
  ...
  /* original assignment; Xun 08/21/17  
  for (i = 0; i < N; i++)
	   so[i] = inData[i];
  */

  /* randomly set the input data, Xun 08/21/17*/
  // set rand seed 
  int rand_seed; 
  ...
  // generate random input data; Xun 08/21/17
  //int KEYSIZE = 100; 
  int KEYSIZE = rand() % 300 + 120; 
  printf("KEYSIZE is %d\n", KEYSIZE);
  word so[KEYSIZE];
  ...
  printf("HEHE");
  //record input size; Xun 08/21/17
  FILE *input_size_file = fopen("./data/gsm/input_size.txt", "a");
